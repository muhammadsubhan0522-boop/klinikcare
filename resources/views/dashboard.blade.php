...
<div class="space-y-8 min-h-screen pb-12">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group hover:border-sky-300 transition-all">
            <div class="space-y-1">
                ...
                <h3 class="text-3xl font-black text-slate-900">{{ $totalPasien ?? 7 }}</h3>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group hover:border-sky-300 transition-all">
            <div class="space-y-1">
                ...
                <h3 class="text-3xl font-black text-slate-900">{{ $totalDokter ?? 8 }}</h3>
                <p class="text-[11px] text-sky-600 font-semibold flex items-center mt-1">
                    <svg class="w-3.5 h-3.5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group hover:border-sky-300 transition-all">
            <div class="space-y-1">
                ...
                <h3 class="text-3xl font-black text-slate-900">{{ $totalAntrean ?? 2 }}</h3>
        ...
        <div class="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between relative overflow-hidden group hover:border-sky-300 transition-all">
            <div class="space-y-1">
                ...
                <h3 class="text-3xl font-black text-slate-900">{{ $totalPemeriksaan ?? 2 }}</h3>
                <p class="text-[11px] text-sky-600 font-semibold flex items-center mt-1">
    ...
    <!-- BAGIAN BAWAH: GRAFIK GARIS (LINE CHART) & ANTREAN AKTIF -->
        ...
        <div class="lg:col-span-4 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm space-y-5 flex flex-col justify-between">
            ...
            <div class="space-y-3">
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between hover:border-sky-200 transition-all">
                    <div>
                        <div class="flex items-center space-x-2">
                            <span class="font-black text-slate-900 text-sm">A-023</span>
                            <span class="text-slate-300">•</span>
                            <span class="font-extrabold text-slate-800 text-xs">Ahmad Fauzi</span>
                        </div>
                        <div class="text-[11px] text-slate-500 mt-0.5">dr. Andi Pratama, Sp.PD</div>
                    </div>
                ...
                <div class="p-4 bg-slate-50 rounded-2xl border border-slate-100 flex items-center justify-between hover:border-sky-200 transition-all">
                    <div>
                        <div class="flex items-center space-x-2">
                            <span class="font-black text-slate-900 text-sm">A-024</span>
                            <span class="text-slate-300">•</span>
                            <span class="font-extrabold text-slate-800 text-xs">Siti Nurhaliza</span>
                        </div>
                        <div class="text-[11px] text-slate-500 mt-0.5">drg. Sarah Melati</div>
                    </div>
                    <span class="px-2.5 py-1 bg-emerald-50 text-emerald-700 border border-emerald-200 rounded-lg text-[10px] font-black uppercase">Dipanggil</span>
                </div>
            </div>
...
<!-- CDN CHART.JS UNTUK GRAFIK GARIS BERANIMASI -->
<script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
<script>
    const ctx = document.getElementById('kunjunganLineChart').getContext('2d');
    
    // Gradient Warna untuk Area Bawah Garis
    ...
    const kunjunganLineChart = new Chart(ctx, {
            ...
            datasets: [
                {
                    ...
                    borderWidth: 3,
                    fill: true,
                    tension: 0.4, // Kurva garis melengkung halus
        ...
        options: {
            ...
            animation: {
                duration: 2000, // Durasi animasi masuk 2 detik
                easing: 'easeInOutQuart'
            },
            plugins: {
                legend: {
                    display: false // Menggunakan legend kustom di atas
                },
                tooltip: {
